...
**The single place all money flows through.** Every deposit and every payout for every module passes through this contract. Fees are deducted here — nowhere else — keeping all fee logic auditable in one place.

---

### Fee Architecture: When, What, Where

**Golden Rule:** Fees are always taken **at the moment funds enter CoreEscrow** (on deposit), never on payout. This means recipients always receive the exact amount shown to them.

```
GROSS AMOUNT (from payer)
       │
       ▼
 PaymentRouter
       │
       ...
       │         │
       │         └── Insurance Cut (10% of fee) ─► CoreEscrow::InsuranceFund
       │
       └─── Net Amount ────────────────────► CoreEscrow Pool
                                             (what gets escrowed / released to recipient)
```

---

### Fee Table — By Module and Sub-Type

| Module | Sub-Type | Fee Rate | Charged On | Charged To | Refunded If Failed? |
|--------|----------|----------|-----------|------------|---------------------|
| **Bounty** | Fixed/FCFS | 5% | Bounty budget at creation | Project/Creator | Yes (on cancel/timeout) |
| **Bounty** | Application + Selection | 5% | Bounty budget at creation | Project/Creator | Yes (on cancel) |
| **Bounty** | Contest | 5% | Full prize pool at creation | Project/Creator | No (pool committed) |
| **Bounty** | Split Bounty | 5% | Full budget at creation | Project/Creator | Partial (unused slots) |
| **Crowdfunding** | Milestone Campaign | 5% | Each pledge at time of pledge | Backer (added on top) | Yes — if campaign fails, full pledge returned, fee NOT returned |
| **Grant** | Milestone Grant | 3% | Total grant budget at creation | Grant poster | No |
| **Grant** | Retrospective | 3% | Matching pool at creation | Grant poster | No |
| **Grant** | Quadratic (QF) — Matching Pool | 3% | Matching pool deposit | Grant poster | No |
| **Grant** | Quadratic (QF) — Donations | 0% | Direct donations from donors | Donors pay no fee | N/A |
| **Hackathon** | Traditional | 4% | Main prize pool at creation | Organizer | No |
| **Hackathon** | Sponsored Track | 4% | Track prize pool at creation | Track sponsor | No |

> **QF Donor Note:** Donors in QF rounds pay zero platform fee. The fee is only charged on the matching pool the organizer deposits. This maximizes signal from the community.

> **Crowdfunding Note:** The 5% fee is charged per pledge (not on total raised) so backers know exactly what they're paying. If the campaign fails and pledges are refunded, **the escrow principal is returned but the fee is not** — this covers platform costs for hosting the campaign.

---

### Fee Split: Every Platform Fee Breaks Into

```
100% of Platform Fee
    ├── 90% → Boundless Treasury Wallet
    └── 10% → CoreEscrow Insurance Fund
               (protects contributors if projects default)
```

**Worked examples:**

```
Bounty: 10,000 XLM budget
  Platform fee (5%)     = 500 XLM
  → Treasury gets       = 450 XLM  (90% of fee)
  → Insurance gets      =  50 XLM  (10% of fee)
  → Escrowed for winner = 9,500 XLM

Crowdfunding pledge: 1,000 XLM
  Platform fee (5%)     = 50 XLM  (charged on top: backer pays 1,050 total)
  → Treasury gets       = 45 XLM
  → Insurance gets      =  5 XLM
  → Escrowed for creator= 1,000 XLM  ← full pledge in escrow
  → If campaign fails: backer gets 1,000 XLM back, 50 XLM fee not returned

Milestone Grant: 50,000 XLM
  Platform fee (3%)     = 1,500 XLM
  → Treasury gets       = 1,350 XLM
  → Insurance gets      =   150 XLM
  → Escrowed for recipient = 48,500 XLM (released per milestone)

Hackathon prize pool: 20,000 XLM
  Platform fee (4%)     = 800 XLM
  → Treasury gets       = 720 XLM
  → Insurance gets      =  80 XLM
  → Escrowed for prizes = 19,200 XLM
    (distributed: 1st = ~11,520, 2nd = ~5,760, 3rd = ~1,920 at 60/30/10 split)
```
...
```rust
struct FeeConfig {
    bounty_fee_bps: u32,          // basis points: 500 = 5%
    crowdfund_fee_bps: u32,       // 500 = 5%
    grant_milestone_fee_bps: u32, // 300 = 3%
    grant_retro_fee_bps: u32,     // 300 = 3%
    grant_qf_fee_bps: u32,        // 300 = 3% (matching pool only)
    hackathon_fee_bps: u32,       // 400 = 4%
    insurance_cut_bps: u32,       // 1000 = 10% of the fee (not of gross)
    treasury: Address,
    admin: Address,
}
...
fn route_deposit(
    ...
    module: ModuleType,
    sub_type: SubType,
) -> i128  // returns net_to_escrow amount
// Step 1: Calculate fee = gross_amount * fee_bps / 10_000
// Step 2: Calculate insurance_cut = fee * insurance_cut_bps / 10_000
// Step 3: treasury_cut = fee - insurance_cut
// Step 4: Transfer treasury_cut → FeeConfig.treasury
// Step 5: CoreEscrow::contribute_insurance(insurance_cut, asset)
// Step 6: Transfer net_to_escrow into the CoreEscrow pool
// Step 7: Store FeeRecord for audit trail
// Step 8: Emit FeeCharged event
// Returns: net_to_escrow (what actually sits in escrow)

// ── CROWDFUNDING PLEDGE (special case) ─────────────────────────────────────
fn route_pledge(
    env: Env,
    backer: Address,
    pool_id: BytesN<32>,
    pledge_amount: i128,   // what backer wants to contribute to the campaign
    asset: Asset,
) -> i128  // returns pledge_amount (full amount escrowed)
// Fee is ADDED ON TOP: backer pays pledge_amount + fee
// Total deducted from backer: pledge_amount * (1 + fee_bps/10_000)
// Full pledge_amount → escrow (refundable if campaign fails)
// Fee portion → treasury + insurance (non-refundable)
// This way: escrow always holds the exact pledge. Refund = exact pledge back.

// ── PAYOUT (no fee deducted here) ─────────────────────────────────────────
fn route_payout(
    env: Env,
    pool_id: BytesN<32>,
    recipient: Address,
...
// No fee at payout — fee already taken at deposit
// Auth: Module registry only
// Calls CoreEscrow::release_partial(pool_id, recipient, amount)
// Emits PayoutRouted event

// ── REFUND (no fee deducted — returns escrowed net amount) ────────────────
fn route_refund(
    env: Env,
    pool_id: BytesN<32>,
    recipient: Address,
...
// Returns exactly what was escrowed (net amount, no fee)
// Bounty cancel: returns net bounty budget to project
// Crowdfund fail: returns each backer's exact pledge_amount
// NOTE: Fee is NOT refunded in any scenario
...
fn update_fee(
    ...
    admin: Address,
    sub_type: SubType,
    new_fee_bps: u32,       // max settable: 1000 bps (10%) hard cap
)
// Admin can adjust fees within hard cap

fn update_insurance_cut(env: Env, admin: Address, new_bps: u32)
// Max: 3000 bps (30% of fee). Min: 500 bps (5% of fee)

fn set_treasury(env: Env, admin: Address, new_treasury: Address)
fn pause_routing(env: Env, admin: Address)       // emergency stop
fn resume_routing(env: Env, admin: Address)

// ── QUERIES ────────────────────────────────────────────────────────────────
fn get_fee_rate(env: Env, sub_type: SubType) -> u32        // returns bps
fn calculate_fee(env: Env, gross: i128, sub_type: SubType) -> (i128, i128)
// Returns (fee_amount, net_to_escrow) — useful for UI preview
fn calculate_pledge_cost(env: Env, pledge: i128) -> i128
// Returns total backer pays including fee
fn treasury_balance(env: Env) -> i128
fn get_fee_record(env: Env, pool_id: BytesN<32>) -> FeeRecord
fn total_fees_collected(env: Env) -> i128
fn total_insurance_contributed(env: Env) -> i128
```

### Events Emitted

```
FeeCharged     { pool_id, sub_type, gross, fee, treasury_cut, insurance_cut, net }
PayoutRouted   { pool_id, recipient, amount, asset }
RefundRouted   { pool_id, recipient, amount, asset }
FeeUpdated     { sub_type, old_bps, new_bps }
TreasuryUpdated { old_treasury, new_treasury }
```

---

### Fee Visibility in UX (What Each User Sees)

| Action | What UI Shows |
|--------|--------------|
| Project posts bounty (10,000 XLM) | "Budget: 10,000 XLM · Platform fee: 500 XLM · Winner receives: 9,500 XLM" |
| Backer pledges 1,000 XLM to campaign | "Your pledge: 1,000 XLM · Platform fee: 50 XLM · Total charged: 1,050 XLM" |
| Creator posts milestone grant (50,000 XLM) | "Grant amount: 50,000 XLM · Platform fee: 1,500 XLM · Recipient receives: 48,500 XLM" |
| Hackathon organizer creates event (20,000 XLM) | "Prize pool: 20,000 XLM · Platform fee: 800 XLM · Total prizes distributed: 19,200 XLM" |
| QF donor donates 500 XLM to a project | "Your donation: 500 XLM · Platform fee: 0 XLM · Project receives: 500 XLM + matching" |

> `calculate_fee()` and `calculate_pledge_cost()` are the functions that power these UI previews — call them before any transaction confirmation screen.
